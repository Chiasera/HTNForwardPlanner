...
namespace Fusion.Fluid
{
    ...
    public class ComplexFluidInteractor : BaseFluidInteractor
    {
        ...
        public override void FluidUpdate()
        {
            ...
            {
                var XZ_position = new Vector2(floater.position.x, floater.position.z);
                float difference = floater.position.y - fluid.GetWaterHeight(XZ_position); /*replace by fluid.GetWaterHeightApproximation(XZ_position) XZ_position = floater XZ position;*/
